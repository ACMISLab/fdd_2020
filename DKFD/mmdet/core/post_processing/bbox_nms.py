...
def multiclass_nms(multi_bboxes,
    ...
    for i in range(dim0):
        ...
        org_score = dets[i][4]
        if flitercross_index[i] and org_score >= 0.10:
            org_x1 = dets[i][0]
            org_y1 = dets[i][1]
            org_x2 = dets[i][2]
            org_y2 = dets[i][3]
    ...
    for i in range(dim0):
        org_score = dets[i][4]
        if flitercontain_index[i] and org_score >= 0.80:
            org_x1 = dets[i][0]
            org_y1 = dets[i][1]
            org_x2 = dets[i][2]
            org_y2 = dets[i][3]
            ...
            while j < dim0 and flitercontain_index[j]:  # 未被定义为存在包含关系或非包含关系的预选框
                if labels[keep[i]] != labels[keep[j]] and org_score - dets[j][4] >= 0.80:  # 标签不同的两个建议框,且两个建议框的得分差值大于20%
