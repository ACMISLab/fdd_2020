...
class BBoxHead(BaseModule):
    ...
    def loss(self,
        ...
        losses = dict()
        # print('--------------------proposal_index--------------------------',proposal_index)
        aug_bboxes = torch.ones((proposal_index[-1], len(bbox_pred[0])), dtype=float, requires_grad=False)
        aug_scores = torch.ones((proposal_index[-1], len(cls_score[0])), dtype=float, requires_grad=False)
        for i in range(len(img_metas)):
            ...
            bboxes, scores = self.get_bboxes(
                rois[proposal_index[i]:proposal_index[i + 1], :],
                cls_score[proposal_index[i]:proposal_index[i + 1], :],
                bbox_pred[proposal_index[i]:proposal_index[i + 1], :],
                img_shape,
                scale_factor,
                rescale=False,
                cfg=None)
            bboxes = bbox_mapping_back(bboxes, img_shape, scale_factor, flip, flip_direction)
            aug_bboxes[proposal_index[i]:proposal_index[i + 1],:] = bboxes
            aug_scores[proposal_index[i]:proposal_index[i + 1]] = scores
        aug_bboxes = aug_bboxes.detach()
        aug_scores = aug_scores.detach()
        # print('-------aug_bboxes------',aug_bboxes)

        # print('-----------------loss-cls_score----------',cls_score)
        # print('-----------------loss-bbox_pred----------',bbox_pred)
